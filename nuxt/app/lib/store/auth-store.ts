...
function createAuthStore() {
	...
	const login = async (credentials: { email: string; password: string }) => {
		try {
			const response: { data: AuthenticationData; sucesss: boolean } = await $fetch('/api/connect/login', {
				method: 'POST',
				body: credentials
			});

			const userResponse: { data: DirectusUser; sucesss: boolean } = await $fetch('/api/connect/get-current-user', {
				method: 'GET',
				headers: {
	...
	const register = async (data: { firstName: string; lastName: string; email: string; password: string }) => {
		try {
			const response: { data: DirectusUser; success: boolean } = await $fetch('/api/connect/signup', {
				method: 'POST',
				body: {
					email: data.email,
